/*
 * Hands on 2 - Get all permanent employees using HQL (with EAGER fetch configuration)
 */
...
public class PermanentEmployeesEagerFetchDemo {
    ...
    static class Department {
        ...
        int getId() { return id; }
        Set<Employee> getEmployeeList() { return employeeList; }
        @Override
        ...
        List<Employee> getAllPermanentEmployees(Logger queryLog) {
            ...
            for (Employee employee : permanentEmployees) {
                ...
                if (departmentsLoaded.add(department.getId())) {
                    queryCount++;
                    queryLog.debug("Query {}: SELECT employees FROM Employee WHERE dp_id = " + department.getId(), queryCount);
                }
            }
            for (Employee employee : permanentEmployees) {
                queryCount++;
                queryLog.debug("Query {}: SELECT skills FROM Skill JOIN employee_skill WHERE em_id = " + employee.id, queryCount);
            }
    ...
    private static final Logger LOGGER = new Logger("PermanentEmployeesEagerFetchDemo");
    ...
    public static void main(String[] args) {
        ...
        it.getEmployeeList().add(john);
        it.getEmployeeList().add(michael);
        hr.getEmployeeList().add(priya);
